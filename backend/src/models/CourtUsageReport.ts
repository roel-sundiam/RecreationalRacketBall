...
const courtUsageReportSchema = new Schema<ICourtUsageReport>(
  {
    ...
    monthlyAmounts: {
      ...
      validate: {
        ...
        message:
          "Invalid monthly amounts: keys must be YYYY-MM format and values must be non-negative",
      },
    },
    totalAmount: {
      type: Number,
...
courtUsageReportSchema.index(
  { clubId: 1, memberName: 1, year: 1 },
  { unique: true },
);
...
courtUsageReportSchema.statics.getReportByYear = function (
  year: number = 2025,
  clubId?: mongoose.Types.ObjectId,
) {
...
courtUsageReportSchema.methods.getMonthAmount = function (
  year: number,
  month: number,
): number {
...
courtUsageReportSchema.methods.setMonthAmount = function (
  year: number,
  month: number,
  amount: number,
): void {
...
courtUsageReportSchema.methods.getAllMonths = function (): Array<{
  key: string;
  amount: number;
  year: number;
  month: number;
...
courtUsageReportSchema.methods.updateMonth = function (
  monthName: string,
  amount: number,
) {
...
const CourtUsageReport = mongoose.model<ICourtUsageReport>(
  "CourtUsageReport",
  courtUsageReportSchema,
);
