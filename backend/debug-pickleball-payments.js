...
async function debugPickleballPayments() {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/recreational-racketball');
    console.log('✅ Connected to MongoDB');

    const db = mongoose.connection.db;
    
    // Find RT2 Pickleball Club
    const club = await db.collection('clubs').findOne({ name: /RT2 Pickleball/i });
    if (!club) {
    ...
    completedPayments.forEach((p, i) => {
      ...
      console.log(`   Full doc: ${JSON.stringify(p, null, 2).substring(0, 500)}...`);
    });
    ...
    const apiPayments = await db.collection('payments')
      .find(apiParams)
      .limit(100)
      .toArray();

    console.log(`✅ API would return: ${apiPayments.length} completed payments`);
