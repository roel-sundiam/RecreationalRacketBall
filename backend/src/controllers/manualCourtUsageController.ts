 ...
 */
export const createManualCourtUsage = asyncHandler(
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    for (const player of players) {
      ...
      const matchedUser = allUsers.find(
        (u) => u.fullName.toLowerCase().trim() === playerName.toLowerCase(),
      );

      if (matchedUser) {
        userId = matchedUser._id.toString();
        console.log(
          `✅ Matched player "${playerName}" to user ${matchedUser.username}`,
        );
      } else {
        console.log(
          `⚠️ No user found for player "${playerName}" - creating payment without userId`,
        );
      ...
      const payment = new Payment({
        ...
        description:
          description ||
          `Court usage fee - ${courtUsageDate.toLocaleDateString()} (${startTime}:00 - ${endTime}:00)`,
        metadata: {
      ...
      console.log(
        `💰 Created pending payment for ${playerName}: ₱${player.amount}`,
      );
    ...
    console.log(
      `✅ Manual court usage recorded successfully - ${createdPayments.length} payments created`,
    );
    ...
    res.status(201).json({
      ...
      data: {
        ...
        totalAmount: players.reduce(
          (sum: number, p: PlayerPayment) => sum + p.amount,
          0,
        ),
 ...
 * @access Private (Superadmin only)
 */
export const getManualCourtUsageHistory = asyncHandler(
  async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    if (!allClubsRequested || !isSuperAdmin) {
      if (req.clubId) {
        query["metadata.clubId"] = req.clubId;
      }
    }
    ...
    for (const payment of manualPayments) {
      ...
      const startTime =
        payment.metadata?.startTime ?? payment.metadata?.timeSlot ?? null;
      const endTime =
        payment.metadata?.endTime ??
        (payment.metadata?.timeSlot ? payment.metadata.timeSlot + 1 : null);
