...
import clubSettingsRoutes from "./routes/clubSettingsRoutes";
...
// CORS configuration (development only - production URLs will be added after deployment)
...
// For future deployment: pattern matching for dynamic URLs
const productionPatterns: RegExp[] = [
  // TODO: Add production URL patterns here when deploying
];
...
app.options("*", (req, res) => {
  ...
  if (!origin) {
    ...
    // Check regex patterns for production deployments
    let matched = false;
  ...
  res.header(
    ...
    "Content-Type, Authorization, Content-Length, X-Requested-With, Accept, X-Club-Id",
  );
...
app.use("/api/clubs", clubRoutes); // Club management routes (public + protected)
...
app.use("/api/rules", rulesRoutes); // Club rules and regulations routes
app.use("/api/club-settings", clubSettingsRoutes); // Club settings (fees, hours, etc.)
...
const startServer = async () => {
  try {
    ...
    console.log(
      "⚠️  Google Sheets sync DISABLED for testing recorded payments integration",
    );
    ...
    httpServer.listen(Number(PORT), "0.0.0.0", () => {
      console.log(
        `🚀 Tennis Club RT2 Backend running on port ${PORT} (NO DATABASE)`,
      );
      console.log(`📱 Environment: ${process.env.NODE_ENV}`);
      console.log(`🔗 Health check: http://localhost:${PORT}/health`);
      console.log(`🔗 Network access: http://192.168.68.113:${PORT}/health`);
