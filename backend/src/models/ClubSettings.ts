...
export interface IClubSettingsDocument extends Omit<IClubSettings, '_id'>, Document {
  _id: mongoose.Types.ObjectId;
}
...
clubSettingsSchema.statics.getByClubId = function(clubId: mongoose.Types.ObjectId) {
  return this.findOne({ clubId });
};

// Static method to create default settings for a new club
clubSettingsSchema.statics.createDefault = async function(clubId: mongoose.Types.ObjectId, updatedBy: mongoose.Types.ObjectId) {
...
const ClubSettings = mongoose.model<IClubSettingsDocument>('ClubSettings', clubSettingsSchema);
