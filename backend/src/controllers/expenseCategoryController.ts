 ...
 */
export const getAllCategoriesIncludingInactive = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const clubFilter = req.clubId
      ? { $or: [{ clubId: req.clubId }, { clubId: { $exists: false } }] }
      : {};
...
export const createCategory = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    const existingCategory = await ExpenseCategory.findOne({
      ...
      ...(clubId ? { $or: [{ clubId }, { clubId: { $exists: false } }] } : {}),
    });
    ...
    const displayOrder = maxOrderCategory
      ? maxOrderCategory.displayOrder + 1
      : 0;
...
export const updateCategory = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (name && name !== category.name) {
      ...
      await Expense.updateMany({ category: category.name }, { category: name });
    }
 ...
 */
export const activateCategory = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
 ...
 */
export const getCategoryUsage = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
