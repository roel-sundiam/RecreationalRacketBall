...
function generateDynamicMonths(year: number): {
  monthNames: string[];
  monthKeys: string[];
} {
...
export const getStaticCourtUsageReport = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const courtUsageRecords = await CourtUsageReport.find(filter).sort({
      totalAmount: -1,
    });
    ...
    console.log(
      `📅 Generated ${monthNames.length} dynamic month columns for year ${year}:`,
      monthNames,
    );
    ...
    console.log(
      `📅 Monthly totals:`,
      monthlyTotals.map((total, i) => `${monthNames[i]}: ₱${total.toFixed(2)}`),
    );
    ...
    console.log(
      `📊 Database report loaded: ${courtUsageRecords.length} members, ₱${totalRevenue.toFixed(2)} total`,
    );
