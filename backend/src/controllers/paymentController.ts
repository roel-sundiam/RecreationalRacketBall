...
// Extend AuthenticatedRequest to include clubId from Express.Request
...
async function getGoogleSheetsBaselineAmount(): Promise<number> {
  try {
    ...
    const courtReceiptsItem = freshData.receiptsCollections.find(
    ...
    console.error(
      "⚠️ Failed to get Google Sheets baseline, using fallback:",
      error,
    );
...
async function updateFinancialReportCourtReceipts(
  clubId: mongoose.Types.ObjectId,
): Promise<void> {
  try {
    console.log(
      "💰 Updating Tennis Court Usage Receipts in financial report...",
    );

    // Calculate total amount from all recorded payments for this club
    ...
    console.log(
      `💰 Total Tennis Court Usage Receipts: ₱${totalCourtReceipts.toFixed(2)}`,
    );
    ...
    const advancesIndex = financialData.receiptsCollections.findIndex(
      (item: any) => item.description === "Advances",
    );
    if (advancesIndex !== -1) {
    ...
    if (courtReceiptsIndex !== -1) {
      const oldAmount =
        financialData.receiptsCollections[courtReceiptsIndex].amount;
      ...
      const appServiceFee = totalCourtReceipts * 0.2;
      ...
      if (appServiceFeeIndex === -1) {
        ...
        console.log(
          `💰 Added App Service Fee to financial report: ₱${appServiceFee.toFixed(2)}`,
        );
      } else {
      ...
      financialData.fundBalance =
        financialData.beginningBalance.amount + financialData.netIncome;
      ...
      console.log(
        `💰 Financial report updated: Court Receipts ₱${oldAmount} → ₱${newAmount} (₱${baselineAmount} baseline + ₱${totalCourtReceipts} recorded)`,
      );
      console.log(
        `💰 New Total Receipts: ₱${financialData.totalReceipts.toLocaleString()}`,
      );
      console.log(
        `💰 New Fund Balance: ₱${financialData.fundBalance.toLocaleString()}`,
      );
      ...
      console.warn(
        "⚠️ Tennis Court Usage Receipts not found in financial report",
      );
    }
  } catch (error) {
...
async function subtractFromCourtUsageReport(
  payment: any,
  clubId: mongoose.Types.ObjectId,
): Promise<void> {
  try {
    console.log(
      "📊 Subtracting from court usage report for unrecorded payment...",
    );
    ...
    if (!memberName) {
      console.warn(
        "⚠️ No member name found for payment, skipping court usage update",
      );
      return;
    }
    ...
    const usageDate =
      payment.paymentDate || payment.metadata?.courtUsageDate || new Date();
    ...
    console.log(
      `📊 Processing ${monthName} ${year} (${monthKey}) for court usage report`,
    );

    console.log(
      `📊 Subtracting ₱${payment.amount} from ${memberName}'s ${monthName} ${year} court usage`,
    );

    // Find the member's court usage record (club-specific)
    ...
    if (!courtUsageRecord) {
      console.warn(
        `⚠️ No court usage record found for ${memberName} (${year}), skipping subtraction`,
      );
      return;
    }
    ...
    console.log(
      `📊 Updated ${memberName}'s ${monthName} from ₱${currentAmount} to ₱${newAmount}`,
    );
    ...
    console.log(
      `✅ Court usage report updated for ${memberName}: -₱${payment.amount} in ${monthName} ${year}`,
    );
...
async function updateCourtUsageReport(
  payment: any,
  clubId: mongoose.Types.ObjectId,
): Promise<void> {
  try {
    ...
    console.log(
      `📊 Updating ${memberName}'s ${monthName} ${year} court usage with ₱${payment.amount}`,
    );

    // Find or create the member's court usage record (club-specific)
    ...
    if (!courtUsageRecord) {
      ...
      console.log(
        `📊 Created new court usage record for ${memberName} (${year})`,
      );
    } else {
      // Update existing record - add to existing amount using Map
      const currentAmount = courtUsageRecord.monthlyAmounts?.get(monthKey) || 0;
      courtUsageRecord.monthlyAmounts.set(
        monthKey,
        currentAmount + payment.amount,
      );
      courtUsageRecord.markModified("monthlyAmounts");
      console.log(
        `📊 Updated ${memberName}'s ${monthName} from ₱${currentAmount} to ₱${currentAmount + payment.amount}`,
      );
    }
...
export const createPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (isManualPayment) {
      ...
      // Verify reservation belongs to the same club
    ...
    if (existingPayment) {
      // If a payment exists, update it instead of creating a new one (for both admin and regular users)
      console.log(
        "💰 Payment exists, updating instead of creating:",
        existingPayment._id,
      );
      ...
      return res.status(200).json({
        ...
        message:
          "Payment updated successfully (existing payment found and updated)",
      });
    }
    ...
    if (isManualPayment) {
      ...
      if (customAmount) {
        ...
        const peakHourFee = pricing.peakHourFee;
        ...
        // Get all active members for player categorization (club-specific)
        // Note: Users are not club-specific in the current schema, but ClubMembership would be
        // For now, this query remains global as User model doesn't have clubId
        ...
        reservation!.players.forEach((player) => {
          ...
          if (isFoundInMembers) {
            memberCount++;
            console.log(
              `💰 Payment: "${playerName}" is a MEMBER (exact match)`,
            );
          } else {
            ...
            for (const memberName of memberNames) {
              ...
              if (
                !matchFound &&
                memberName.includes(cleanPlayerName) &&
                cleanPlayerName.length > 2
              ) {
                bestMatch = memberName;
                matchFound = true;
                console.log(
                  `💰 Payment: "${playerName}" is a MEMBER (partial match: "${bestMatch}")`,
                );
                break;
              }
              ...
              if (!matchFound) {
                ...
                if (wordMatch) {
                  ...
                  matchFound = true;
                  console.log(
                    `💰 Payment: "${playerName}" is a MEMBER (word match: "${bestMatch}")`,
                  );
                  break;
                }
              }
            ...
            if (matchFound && bestSimilarity > 0.6) {
              memberCount++;
              console.log(
                `💰 Payment: "${playerName}" is a MEMBER (fuzzy match: "${bestMatch}", similarity: ${bestSimilarity.toFixed(2)})`,
              );
            } else if (matchFound) {
              memberCount++;
            } else {
              nonMemberCount++;
              console.log(
                `💰 Payment: "${playerName}" is a NON-MEMBER (no match found)`,
              );
            }
          }
        });
        ...
        if (isPeakHour) {
          ...
          const calculatedFee =
            memberCount * offPeakFeePerMember +
            nonMemberCount * offPeakFeePerNonMember;
      ...
      if (reservationDate.getTime() === today.getTime()) {
        ...
        dueDate.setTime(
          Math.min(
            ...
            oneDayBeforeReservation.getTime(),
          ),
        );
      }
    }
    ...
    if (
      payForUserIds &&
      Array.isArray(payForUserIds) &&
      payForUserIds.length > 0
    ) {
      console.log(
        `💰 Multi-member payment requested for ${payForUserIds.length} members`,
      );
      ...
      console.log(
        `💰 Processing payments for ${payForUserIds.length} users:`,
        payForUserIds,
      );
      ...
      for (const userId of payForUserIds) {
        ...
        console.log(
          `💰 Processing payment for user: ${targetUser?.fullName} (${userId})`,
        );

        // Check if this user already has a pending payment
        const existingPayment = existingPendingPayments.find(
          (p) => p.userId.toString() === userId,
        );
        console.log(
          `💰 Existing pending payment found for ${targetUser?.fullName}:`,
        ...
        if (!isManualPayment && reservation) {
          ...
          const members = reservation.players.filter(
            (p: any) => typeof p === "object" && p.isMember,
          );
          const guests = reservation.players.filter(
            (p: any) => typeof p === "object" && p.isGuest,
          );
        ...
        if (existingPayment) {
          ...
          console.log(
            `💰 Updating existing pending payment for ${targetUser?.fullName}`,
          );
          ...
          if (!isPayingForSelf) {
            ...
            existingPayment.notes =
              notes ||
              `Paid by ${req.user.fullName} on behalf of ${targetUser?.fullName}`;
          ...
          if (isManualPayment) {
            ...
            paymentData.metadata = {
              ...
              originalDebtor: isPayingForSelf
                ? undefined
                : targetUser?.fullName,
            };
          } else {
        ...
        console.log(
          `💰 Added to createdPayments. Total so far: ${createdPayments.length}`,
        );
      }
    ...
    let paymentData: any = {
      ...
      notes:
        notes ||
        (isAdminOverride
          ? `Admin override: Custom amount ₱${paymentAmount.toFixed(2)} set by ${req.user.username}`
          : undefined),
    };
        ...
        return {
          ...
          reservationId: reservation
            ? {
                ...
                date: reservation.date,
                timeSlot: reservation.timeSlot,
                players: Array.isArray(reservation.players)
                  ? reservation.players
                  : [],
...
export const getPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Verify payment belongs to the same club
    if (payment.clubId?.toString() !== req.clubId?.toString()) {
      return res.status(404).json({
        success: false,
        error: "Payment not found",
      });
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (payment.reservationId) {
        ...
        if (
          reservation &&
          reservation.clubId?.toString() === req.clubId?.toString()
        ) {
          console.log(
            "💰 Updating reservation payment status from",
      ...
      try {
        ...
        if (webSocketService.isInitialized()) {
          const memberName =
            (payment.userId as any)?.fullName || "Unknown Member";
          ...
          console.log(
            "📡 Real-time court usage update broadcasted for payment completion",
          );
        }
      } catch (error) {
        console.error(
          "⚠️ Failed to broadcast payment completion update:",
          error,
        );
      ...
      if (payment.status === "completed") {
        console.log(
          "⚠️ Payment was marked completed but reservation update may have failed",
        );
        return res.status(200).json({
          success: true,
          data: payment,
          message:
            "Payment processed successfully (reservation update may have failed)",
          warning:
            "Reservation status update encountered an error but payment is completed",
        });
      }
...
export const updatePayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (payment.metadata && !payment.metadata.discounts) {
      ...
      payment.metadata = { discounts: [] };
      console.log(
        "💰 MIGRATION: Created missing metadata with discounts array for payment",
        id,
      );
    }
      ...
      if (["completed", "record"].includes(payment.status)) {
        if (newDate > new Date()) {
          return res.status(400).json({
            success: false,
            error:
              "Payment date cannot be in the future for completed payments",
          });
        }
      }
    ...
    try {
      ...
      console.error("💰 ERROR stack:", error.stack);
      console.error(
        "💰 Poll document causing error:",
        payment.pollId ? JSON.stringify(payment.pollId, null, 2) : "No pollId",
      );
      throw error;
    }
...
export const getMyPayments = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    // Add clubId filter only if provided (not required for members viewing their own payments)
    if (req.clubId) {
    ...
    try {
      const payments = await Payment.find(filter)
        ...
        .populate({
          path: "reservationId",
          select:
            "userId date timeSlot endTimeSlot duration players status totalFee",
...
export const getOverduePayments = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const checkMyOverduePayments = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
      return res.status(401).json({
        success: false,
...
export const recordPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      await updateCourtUsageReport(payment, req.clubId!);
      ...
      try {
        ...
        if (webSocketService.isInitialized()) {
          ...
          webSocketService.emitCourtUsageUpdate({
            ...
            data: {
              summary: {
                message: "Payment recorded - refresh report to see changes",
              },
            },
            timestamp: new Date().toISOString(),
          ...
          console.log(
            "📡 Real-time court usage update broadcasted for payment recording",
          );
        }
      } catch (error) {
        console.error(
          "⚠️ Failed to broadcast payment recording update:",
          error,
        );
...
export const unrecordPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const { notes } = req.body;
    ...
    try {
      ...
      await subtractFromCourtUsageReport(payment, req.clubId!);
      ...
      try {
        ...
        if (webSocketService.isInitialized()) {
          ...
          webSocketService.emitCourtUsageUpdate({
            ...
            data: {
              summary: {
                message: "Payment unrecorded - refresh report to see changes",
              },
            },
            timestamp: new Date().toISOString(),
          ...
          console.log(
            "📡 Real-time court usage update broadcasted for payment unrecording",
          );
        }
      } catch (error) {
        console.error(
          "⚠️ Failed to broadcast payment unrecording update:",
          error,
        );
...
export const calculatePaymentAmount = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const cleanupDuplicatePayments = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    console.log(
      ...
      "reservations with duplicate payments",
    );
    ...
    for (const group of duplicateGroups) {
      ...
      if (completedPayments.length > 0 && pendingPayments.length > 0) {
        console.log(
          `✅ Keeping ${completedPayments.length} completed, removing ${pendingPayments.length} pending`,
        );
...
export const createPaymentValidation = [
  ...
  body("playerNames")
    ...
    .custom((playerNames) => {
      if (
        !Array.isArray(playerNames) ||
        playerNames.some((name) => typeof name !== "string" || !name.trim())
      ) {
...
export const payOnBehalf = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const {
      reservationId,
      payForUserId,
    ...
    if (reservationDate.getTime() === today.getTime()) {
      ...
      dueDate.setTime(
        Math.min(sevenDaysFromNow.getTime(), oneDayBeforeReservation.getTime()),
      );
    }
    ...
    // Calculate if peak hour using club-specific pricing
    const pricing = await PricingService.getClubPricing(req.clubId!);
    const peakHours = pricing.peakHours;
    const isPeakHour = peakHours.includes(reservation.timeSlot);
    ...
    if (paidCount >= membersInReservation.length) {
      reservation.paymentStatus = "paid";
      console.log(
        '✅ All members have paid, updating reservation status to "paid"',
      );
    }
...
export const recordMembershipFeePayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const {
      userId,
      membershipYear,
    ...
    console.log("💳 Recording membership fee payment:", {
      userId,
      membershipYear,
      amount,
      paymentMethod,
    ...
    if (
      ...
      user.membershipYearsPaid.includes(membershipYear)
    ) {
      return res.status(400).json({
        success: false,
    ...
    console.log(
      `✅ Membership fee payment recorded for ${user.fullName} - Year ${membershipYear}`,
    );
...
export const getMembershipPayments = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { year, userId, status } = req.query;

    const filter: any = { clubId: req.clubId, paymentType: "membership_fee" };
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          ...
          years: [...new Set(payments.map((p) => p.membershipYear))].sort(
            (a, b) => (b || 0) - (a || 0),
          ),
        },
      },
...
export const getMembershipPaymentSummary = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
 ...
 */
export const updateMembershipPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    const { amount, membershipYear, paymentMethod, paymentDate, notes } =
      req.body;
    ...
    console.log(
      `💳 Membership payment updated: ${payment._id} by ${req.user!.username}`,
    );
 ...
 */
export const deleteMembershipPayment = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    ...
    console.log(
      `🗑️ Membership payment deleted: ${id} (Year: ${membershipYear}) by ${req.user!.username}`,
    );
...
async function updateFinancialReportMembershipFees(): Promise<void> {
  try {
    ...
    result.forEach(({ _id: year, totalAmount }) => {
      const existingIndex = financialData.receiptsCollections.findIndex(
        (item: any) => item.description === `Annual Membership Fees ${year}`,
      );

      if (existingIndex !== -1) {
        // Update existing entry
        const oldAmount =
          financialData.receiptsCollections[existingIndex].amount;
        financialData.receiptsCollections[existingIndex].amount = totalAmount;
        console.log(
          `💰 Updated: Annual Membership Fees ${year}: ₱${oldAmount} → ₱${totalAmount.toFixed(2)}`,
        );
      } else {
        // Add new entry
        ...
        console.log(
          `💰 Added: Annual Membership Fees ${year}: ₱${totalAmount.toFixed(2)}`,
        );
      }
    });
    ...
    console.error(
      "❌ Error updating financial report for membership fees:",
      error,
    );
    throw error;
  }
