  ...
  try {
    ...
    if (!userId || !sessionId || !action || !component) {
      return res.status(400).json({
        success: false,
        message:
          "Missing required fields: userId, sessionId, action, component",
      });
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Debug: Check if we have club memberships in the database
      const ClubMembership = mongoose.model("ClubMembership");
      const membershipCount = await ClubMembership.countDocuments({
        status: "approved",
      });
      console.log(
        "📊 Analytics: Total approved memberships in database:",
        membershipCount,
      );

      // Get user visit counts - exclude superadmin users
      console.log(
        "📊 Analytics: Fetching user visit counts with club information...",
      );
      const userVisitCounts = await PageView.aggregate([
              ...
              $arrayElemAt: [
                {
                  $filter: {
                    input: "$memberships",
                    as: "membership",
                    cond: { $eq: ["$$membership.status", "approved"] },
                  },
                },
                0,
              ],
        ...
        },
        {
          $lookup: {
            from: "clubs",
            localField: "primaryMembership.clubId",
            foreignField: "_id",
            as: "club",
          },
        },
      ...
      console.log(
        "📊 Analytics: User visit counts result:",
        JSON.stringify(userVisitCounts.slice(0, 2), null, 2),
      );
      ...
      const analyticsData = {
        ...
        engagement: {
          ...
          avgActions:
            totalSessions > 0
              ? Math.round((totalUserActivity / totalSessions) * 10) / 10
              : 0,
  ...
  async (req, res) => {
    try {
      ...
      if (dateFrom || dateTo) {
        ...
        if (dateFrom)
          matchConditions.timestamp.$gte = new Date(dateFrom as string);
