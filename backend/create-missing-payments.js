...
async function createMissingPayments() {
  try {
    ...
    for (const reservation of reservations) {
      ...
      console.log(`   💰 Pricing: ${pricingModel}, Base: ₱${totalBaseFee}, Guests: ₱${totalGuestFee}`);
      ...
      for (const member of members) {
        ...
        console.log(`   💳 Created payment for ${member.name}: ₱${paymentAmount} (${paymentStatus})`);
      }

      // Update reservation with payment IDs
      reservation.paymentIds = paymentIds;
      if (paymentIds.every(async id => {
      ...
      console.log(`   ✅ Updated reservation with ${paymentIds.length} payment IDs`);
    }
