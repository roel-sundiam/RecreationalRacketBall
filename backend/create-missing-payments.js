...
async function createMissingPayments() {
  try {
    ...
    console.log(`\n🔍 Finding reservations for ${user.fullName} (${user._id})`);

    // Find all active reservations without payments
    const reservations = await Reservation.find({
      userId: user._id.toString(),
      status: { $in: ["pending", "confirmed"] },
      $or: [{ paymentIds: { $exists: false } }, { paymentIds: { $size: 0 } }],
    }).sort({ date: -1 });

    console.log(
      `\n📋 Found ${reservations.length} reservations without payments`,
    );

    for (const reservation of reservations) {
      console.log(
        `\n📅 Processing reservation: ${reservation.date.toISOString().split("T")[0]} ${reservation.timeSlot}:00`,
      );
      ...
      const hasNewPlayerFormat =
        ...
        "isMember" in reservation.players[0];

      if (!hasNewPlayerFormat) {
        console.log(`   ⏭️  Skipping - old player format (no member tracking)`);
        continue;
      }

      const members = reservation.players.filter((p) => p.isMember);
      const guests = reservation.players.filter((p) => p.isGuest);

      console.log(`   👥 ${members.length} members, ${guests.length} guests`);

      if (members.length === 0) {
        console.log(`   ⏭️  Skipping - no members`);
        continue;
      }
      ...
      if (pricingModel === "fixed-daily") {
        ...
        for (
          ...
          totalBaseFee += isPeakHour
            ? pricing.peakHourFee
            : pricing.offPeakHourFee;
          totalGuestFee += guests.length * pricing.guestFee;
        }
        memberShare = totalBaseFee / members.length;
      }
      ...
      console.log(
        `   💰 Pricing: ${pricingModel}, Base: ₱${totalBaseFee}, Guests: ₱${totalGuestFee}`,
      );
      ...
      for (const member of members) {
        ...
        console.log(
          `   💳 Created payment for ${member.name}: ₱${paymentAmount} (${paymentStatus})`,
        );
      }
      ...
      console.log(
        `   ✅ Updated reservation with ${paymentIds.length} payment IDs`,
      );
    }
