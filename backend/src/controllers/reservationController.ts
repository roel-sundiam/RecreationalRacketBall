...
async function convertPlayersToObjects(playerNames: string[]): Promise<any[]> {
  ...
  const memberMap = new Map(
    allMembers.map((m) => [m.fullName.toLowerCase().trim(), m._id.toString()]),
  );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (updateResult.modifiedCount > 0) {
        console.log(
          `🔄 Auto-marked ${updateResult.modifiedCount} past pending reservation(s) as 'no-show'`,
        );
      }
    } catch (error) {
...
export const getReservationsForDate = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    console.log(
      `🔍 QUERY DEBUG: Requested date: ${date}, Parsed queryDate: ${queryDate.toISOString()}`,
    );
    if (excludeId) {
      console.log(
        `🔍 EDIT MODE: Excluding reservation ID ${excludeId} from availability checks`,
      );
    }
    ...
    if (!reservations || !Array.isArray(reservations)) {
      console.warn(
        `⚠️ getReservationsForDate returned invalid data:`,
        reservations,
      );
      reservations = [];
    }
    ...
    if (excludeId) {
      reservations = reservations.filter(
        (r: any) => r._id.toString() !== excludeId,
      );
      console.log(
        `🔍 EDIT MODE: After excluding ${excludeId}, ${reservations.length} reservations remain`,
      );
    }

    console.log(
      `🔍 QUERY DEBUG: Found ${reservations.length} reservations for date ${date}`,
    );
    ...
    const openPlayEvents = allPossibleEvents.filter((event) => {
      ...
      const phEventDate = eventDate.toLocaleDateString("en-CA", {
    ...
    openPlayEvents.forEach((event) => {
      if (
        event?.openPlayEvent?.blockedTimeSlots &&
        Array.isArray(event.openPlayEvent.blockedTimeSlots)
      ) {
        event.openPlayEvent.blockedTimeSlots.forEach((slot) =>
    ...
    for (let hour = 5; hour <= 22; hour++) {
      ...
      if (hour === 22) {
        ...
        canBeEndTime = !reservations.find(
          (r: any) =>
      ...
      const openPlayEvent = isBlockedByOpenPlay
        ? openPlayEvents.find(
            (event) =>
              event.openPlayEvent?.blockedTimeSlots &&
              Array.isArray(event.openPlayEvent.blockedTimeSlots) &&
              event.openPlayEvent.blockedTimeSlots.includes(hour),
          )
        : null;
      ...
      if (hour === 17 || hour === 21 || hour === 22) {
        ...
        console.log(
          `  - Available for END: ${canBeEndTime && !isBlockedByOpenPlay}`,
        );
      }
      ...
      const slotData = {
        ...
        openPlayEvent: openPlayEvent
          ? {
              ...
              confirmedPlayers:
                openPlayEvent.openPlayEvent?.confirmedPlayers?.length || 0,
...
export const getReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (
      ...
      reservationUserId?.toString() !== req.user?._id?.toString()
    ) {
      res.status(403).json({
        success: false,
        error: "Access denied",
...
export const createReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (totalOverdue > 0) {
      ...
      overdueReservations.forEach((r) => {
        const daysOverdue = Math.ceil(
          (Date.now() - r.date.getTime()) / (1000 * 60 * 60 * 24),
        );
        overdueDetails.push({
          id: r._id,
          amount: r.totalFee || 0,
      ...
      console.log(
        `⚠️ User ${req.user.username} has ${totalOverdue} overdue payment(s) (${overduePayments.length} payments + ${overdueReservations.length} unpaid reservations), blocking reservation`,
      );
      ...
      res.status(403).json({
        ...
        message:
          "You have pending payments that are overdue. Please settle them before making a new reservation.",
    ...
    const operatingHours = await PricingService.getClubOperatingHours(
      req.clubId!,
    );
    ...
    const isAvailable = await (Reservation as any).isSlotRangeAvailable(
      reservationDate,
      timeSlot,
      endTimeSlot,
    );
    ...
    if (openPlayEvent) {
      const blockedSlots =
        openPlayEvent.openPlayEvent?.blockedTimeSlots?.filter((slot: number) =>
          slotsToCheck.includes(slot),
        ) || [];
      const blockedSlotsDisplay = blockedSlots
        .map((slot: number) => `${slot}:00-${slot + 1}:00`)
        .join(", ");
    ...
    if (!finalTotalFee) {
      ...
      const peakHours = (process.env.PEAK_HOURS || "5,18,19,20,21")
        .split(",")
        .map((h) => parseInt(h));
    ...
    if (user.creditBalance >= finalTotalFee) {
      try {
        console.log(
          `💳 Auto-deducting ₱${finalTotalFee} from ${user.fullName}'s credit balance (₱${user.creditBalance})`,
        );

        const creditTransaction = await (
          CreditTransaction as any
        ).createTransaction(
          req.user._id,
          "deduction",
          finalTotalFee,
        ...
        console.log(
          `✅ Successfully deducted ₱${finalTotalFee} from credits. New balance: ₱${user.creditBalance - finalTotalFee}`,
        );
      } catch (error) {
    ...
    if (members.length > 0) {
      ...
      for (const member of members) {
        ...
        if (pricingModel === "fixed-daily") {
          ...
          if (existingDailyPayment && existingDailyPayment.reservationId) {
            const existingResDate = new Date(
              (existingDailyPayment.reservationId as any).date,
            );
        ...
        const guestFeeForReserver =
          pricingModel === "fixed-daily" && alreadyPaidDaily
            ? 0
            : totalGuestFee;
        const paymentAmount =
          baseAmount + (isReserver ? guestFeeForReserver : 0);
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (reservationDateTime <= now) {
      res.status(400).json({
        success: false,
        error:
          "Cannot edit reservations that have already started or passed. Payments can be made after the reservation time.",
      });
      return;
    }
    ...
    if (date || timeSlot) {
      ...
      const isAvailable = await (Reservation as any).isSlotAvailable(
        newDate,
        newTimeSlot,
        id,
      );
    ...
    if (players) {
      ...
      if (reservation.paymentIds && reservation.paymentIds.length > 0) {
        ...
        await Payment.updateMany(
          {
            clubId: req.clubId,
            _id: { $in: reservation.paymentIds },
            status: "pending",
          },
          { $set: { status: "cancelled" } },
        );
        console.log(
          `📝 Cancelled ${reservation.paymentIds.length} pending payments for reservation ${id}`,
        );
      }
      ...
      if (members.length > 0) {
        ...
        const calculatedEndTimeSlot =
          reservation.endTimeSlot ||
          reservation.timeSlot + reservation.duration;
        let totalBaseFee = 0;
        let totalGuestFee = 0;
        ...
        for (const member of members) {
          const isReserver = member.userId === reserverId;
          const paymentAmount = isReserver
            ? memberShare + totalGuestFee
            : memberShare;
        ...
        await reservation.save({ validateBeforeSave: false });
        console.log(
          `✅ Created ${paymentIds.length} new payments for reservation ${id}`,
        );
      }
    } else if (
      !players &&
      (endTimeSlot !== undefined || duration !== undefined)
    ) {
      // No player changes but duration/time changed - need to recalculate fee and update payments
      console.log(
        "🔄 Duration/time changed without player changes - recalculating fees",
      );
      ...
      if (members.length > 0) {
        ...
        for (const member of members) {
          const memberUserId =
            typeof member === "string" ? null : member.userId;
        ...
        await reservation.save({ validateBeforeSave: false });
        console.log(
          `✅ Created ${paymentIds.length} new payments for updated reservation ${id}`,
        );
      }
    } else if (!players) {
...
export const cancelReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (
      ...
      console.log(
        `🚫 Access denied - User ${req.user._id} trying to cancel reservation owned by ${reservation.userId}`,
      );
      res.status(403).json({
        success: false,
        error: "Access denied",
    ...
    if (isToday && currentHour >= reservationHour) {
      console.log(
        `⚠️  Same-day cancellation for ongoing/past time slot - allowing for emergency/weather reasons`,
      );
    }
    ...
    if (
      ...
      console.log(
        `❌ Cannot cancel - reservation status is: ${reservation.status}`,
      );
      res.status(400).json({
        success: false,
    ...
    if (reservation.paymentStatus === "paid") {
      ...
      if (creditTransaction) {
        ...
        try {
          console.log(
            `💳 Auto-refunding ₱${creditRefundAmount} credits for cancelled reservation`,
          );
          ...
          console.log(
            `✅ Successfully refunded ₱${creditRefundAmount} to user's credit balance`,
          );
        } catch (error) {
          console.error(
            "Failed to auto-refund credits for cancelled reservation:",
            error,
          );
    ...
    try {
      ...
      console.log(
        `🗑️  Deleted ${deletedPayments.deletedCount} pending payment(s) for cancelled reservation`,
      );
        ...
        for (const deletedPayment of paymentsToDelete) {
          if (deletedPayment.amount > 0) {
            ...
            const sameDayRecordPayment = await Payment.findOne({
              ...
              amount: 0,
              "metadata.date": {
                $gte: reservationStartOfDay,
                $lte: reservationEndOfDay,
              },
            ...
            if (sameDayRecordPayment) {
              ...
              sameDayRecordPayment.description =
                sameDayRecordPayment.description
                  .replace("(Daily fee already covered)", "")
                  .trim();
              await sameDayRecordPayment.save();
              console.log(
                `✅ Converted record payment to pending (₱${FIXED_DAILY_FEE}) for same-day reservation after cancellation`,
              );
            }
          }
        }
      ...
      console.error(
        "Failed to delete payments for cancelled reservation:",
        error,
      );
...
export const completeReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const getMyUpcomingReservations = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
      res.status(401).json({
        success: false,
    ...
    const reservations = await Reservation.find({
      ...
      status: {
        $in: ["pending", "confirmed", "cancelled", "completed", "no-show"],
      },
    }).sort({ date: 1, timeSlot: 1 });
...
export const blockCourt = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const {
      date,
      timeSlot,
    ...
    console.log(
      `🚫 Court blocked: ${date} ${timeSlot}:00-${endTimeSlot}:00 by ${req.user.username} (${blockReason})`,
    );
...
export const getBlockedReservations = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
...
export const updateBlockedReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
...
export const deleteBlockedReservation = asyncHandler(
  async (req: AuthenticatedRequest, res: Response) => {
    const { id } = req.params;
    ...
    console.log(
      `🗑️ Court block removed: ${reservation.date} ${reservation.timeSlot}:00`,
    );
